...
def process_single_image(input_image, prompt, ratio, resolution, x_position, y_position):
    """使用二值蒙版进行AI扩图生成"""
    # 获取可视化蒙版、二值蒙版和调整后的图像
    ...
    # # 来指导AI生成需要扩展的区域
    # image = Image.new('RGB', (mask_width, mask_height))
    ...
    # # 将调整后的原图粘贴到指定位置
    # image.paste(resized_image, (x, y))
    
    return image
    
def resample_video(video_path, fps=15):
    """
    使用ffmpeg将视频转换为15fps并替换原视频
    ...
    if ext not in video_extensions:
        print(f"错误: '{video_path}' 不是支持的视频文件")
        return
    
    try:
        # 创建临时文件来存储处理后的视频
        temp_dir = tempfile.gettempdir()
        temp_file = os.path.join(temp_dir, f"temp_processed{ext}")
        
        # 使用ffmpeg将视频转换为15fps
        # -y 选项表示覆盖输出文件而不提示
        # -r 15 设置输出帧率为15fps
        # -crf 23 是x264的恒定速率因子，值越小质量越好（范围0-51）
        # -preset medium 平衡编码速度和压缩效率
        command = [
            'ffmpeg',
            '-i', video_path,         # 输入文件
            '-y',                     # 覆盖输出文件
            '-r', str(fps),               # 设置帧率为15fps
            '-crf', '23',             # 视频质量参数
            '-preset', 'medium',      # 编码预设
        ...
        print(f"成功将视频转换为{fps}fps并替换原文件: {video_path}")
...
def expand_image(input_image, prompt, ratio, resolution, x_position, y_position, video_path, is_video):
    if is_video:
        resample_video(video_path, 6)
        # 打开视频文件
        ...
        # 生成蒙版和相关参数
        visualization_mask, binary_mask_pil, resized_image = generate_mask_visualization_and_binary(
            first_frame_pil, ratio, resolution, x_position, y_position
        )
        
        # 获取蒙版尺寸和调整后的图像尺寸
        mask_width, mask_height = RATIO_RESOLUTIONS[ratio]
        resized_width, resized_height = visualization_mask.size
        ...
        out = cv2.VideoWriter(temp_video_path, fourcc, fps, (mask_width, mask_height))
        
        # 处理第一帧
        processed_frame = process_single_image(first_frame_pil, prompt, ratio, resolution, x_position, y_position)
        open_cv_image = cv2.cvtColor(np.array(processed_frame), cv2.COLOR_RGB2BGR)
        ...
        for _ in range(total_frames - 1):
            ...
            processed_frame = process_single_image(frame_pil, prompt, ratio, resolution, x_position, y_position)
            open_cv_image = cv2.cvtColor(np.array(processed_frame), cv2.COLOR_RGB2BGR)
            out.write(open_cv_image)
        ...
        try:
            ...
            #         acodec='aac',     # 音频编码器
            #         r=fps,          # 保持原帧率
            #         shortest=None     # 确保音视频长度一致
            #     )
            #     .overwrite_output()
            #     .run()    # quiet=True 可以隐藏ffmpeg输出信息
            # )
            ...
            (
                ffmpeg
                .concat(
                    ffmpeg.input(temp_video_path).video,
                    audio_stream,
                    v=1, a=1
                )
                .output(output_path, vcodec='libx264', acodec='aac', r=fps)
                .overwrite_output()
                .run()
            )
        ...
        return process_single_image(input_image, prompt, ratio, resolution, x_position, y_position), None
...
with gr.Blocks(title="AI图像扩展工具") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            output_video = gr.Video(label="扩展结果", autoplay=True, format="mp4")
    ...
    def update_image(video_path):
        ...
        first_frame_rgb = cv2.cvtColor(first_frame, cv2.COLOR_BGR2RGB)
        first_frame_pil = Image.fromarray(first_frame_rgb)
        return first_frame_pil, True

        
    input_video.change(
        fn=update_image,
        inputs=[input_video],
        outputs=[input_image, is_video]
    )
    
    # 生成按钮点击事件
    generate_btn.click(
        fn=expand_image,
        inputs=[input_image, prompt, ratio, input_resolution, x_position, y_position, input_video, is_video],
